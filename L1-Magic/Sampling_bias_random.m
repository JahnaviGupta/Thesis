...
alpha = 1;
prob_map = 1 ./ (1 + dist_sq.^alpha);

prob_vec = prob_map(:);
prob_vec = prob_vec / sum(prob_vec);

M = round(rand_percent * N * N);

% ----- CUSTOM WEIGHTED SAMPLING (no toolbox) -----
cdf = cumsum(prob_vec);
selected_idx = zeros(M,1);
...
while count < M
    ...
    % avoid duplicates
    if ~ismember(idx, selected_idx)
        count = count + 1;
        selected_idx(count) = idx;
    end
end

final_order = [L(selected_idx), P(selected_idx)];
